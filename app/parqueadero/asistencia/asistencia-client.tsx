...
import {
  ...
  ShieldCheck, ShieldOff, RotateCcw, Users, QrCode, Pencil, Loader2
} from "lucide-react"
    ...
    while (i < recs.length) {
      ...
      if (r.tipo === "INGRESO") {
        ...
        jornadas.push({
          ...
          ingreso_id:       r.id,
          salida_id:        siguiente?.tipo === "SALIDA" ? siguiente.id : null,
        })
...
// Convierte timestamp ISO a formato datetime-local en hora Colombia
...
interface EditSalidaState {
  jornadaKey:  string
  usuario_id:  string
  salida_id:   string | null  // null = crear nueva salida
  timestamp:   string         // datetime-local
}
...
export default function AsistenciaClient({ puedeEditar }: { puedeEditar: boolean }) {
  ...
  // Modal edición salida
  const [editSalida, setEditSalida]   = useState<EditSalidaState | null>(null)
  ...
  function abrirEditSalida(j: Jornada) {
    setEditSalida({
      jornadaKey: j.key,
      usuario_id: j.usuario_id,
      salida_id:  j.salida_id,
      timestamp:  j.salida ? tsToDatetimeLocal(j.salida) : tsToDatetimeLocal(j.ingreso ?? new Date().toISOString()),
    })
  }

  async function guardarSalida() {
    if (!editSalida) return
    setGuardandoSalida(true)
    try {
      const isoTimestamp = `${editSalida.timestamp}:00-05:00`

      let res: Response
      if (editSalida.salida_id) {
        res = await fetch(`/api/parqueadero/asistencia/${editSalida.salida_id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ timestamp: isoTimestamp }),
        })
      } else {
        res = await fetch("/api/parqueadero/asistencia", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ usuario_id: editSalida.usuario_id, timestamp: isoTimestamp }),
        })
      }
      ...
      toast.success(editSalida.salida_id ? "Salida actualizada" : "Salida registrada")
      setEditSalida(null)
  ...
  function renderJornadaRow(j: Jornada, key: string) {
    return (
      <tr key={key} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
        ...
        <td className="px-4 py-3 tabular-nums text-rose-600 font-medium">
          <div className="flex items-center gap-2">
            ...
            {puedeEditar && j.ingreso && (
              <button
                onClick={() => abrirEditSalida(j)}
                title={j.salida ? "Editar hora de salida" : "Registrar salida"}
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="rounded-lg border overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                ) : filtrados.map((j) => renderJornadaRow(j, j.key))}
              </tbody>
            </table>
          </div>
        </TabsContent>
          ...
          <div className="rounded-lg border overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                ) : filtradosHist.map((j) => renderJornadaRow(j, j.key + "-h"))}
              </tbody>
            </table>
          </div>
        </TabsContent>
      ...
      {/* ── MODAL EDICIÓN SALIDA ── */}
      <Dialog open={!!editSalida} onOpenChange={open => { if (!open) setEditSalida(null) }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {editSalida?.salida_id ? "Editar hora de salida" : "Registrar salida"}
            </DialogTitle>
          </DialogHeader>
          {editSalida && (
            <div className="space-y-4 py-2">
              <div className="space-y-1.5">
                ...
                <Input
                  type="datetime-local"
                  value={editSalida.timestamp}
                  onChange={e => setEditSalida({ ...editSalida, timestamp: e.target.value })}
                />
              </div>
              <p className="text-xs text-muted-foreground">
                Hora en zona horaria Colombia (UTC−5).
              </p>
            </div>
          )}
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setEditSalida(null)}>Cancelar</Button>
            <Button onClick={guardarSalida} disabled={guardandoSalida}>
              {guardandoSalida
